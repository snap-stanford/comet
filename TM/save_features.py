...
#        modelfile   = get_resume_file(checkpoint_dir) #comment in 2019/08/03 updates as the validation of baseline/baseline++ is added
    ...
    if params.save_iter != -1:
        outfile = os.path.join( checkpoint_dir.replace("checkpoints","features"), 'test' + "_" + str(params.save_iter)+ ".hdf5") 
    else:
        outfile = os.path.join( checkpoint_dir.replace("checkpoints","features"), 'test' + ".hdf5") 

    datamgr         = SimpleDataManager(batch_size = 64)
    base_loader     = datamgr.get_data_loader(root='./filelists/tabula_muris', mode='test')
    ...
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    save_features(model, base_loader, outfile)